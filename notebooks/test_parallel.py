...
def test_exiffi(X_train,X_test,X,n_runs=10,seed=None,parallel=True):
    ...
    for i in trange(n_runs):
        ...
        if parallel:
            EDIFFI=Extended_DIFFI_parallel(300,max_depth=100,subsample_size=256,plus=1,seed=seed+i)
            EDIFFI.set_num_processes(8)
        else:
            EDIFFI=Extended_DIFFI_original(300,max_depth=100,subsample_size=256,plus=1,seed=seed+i)
